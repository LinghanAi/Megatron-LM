...
from .QFuncs.nf4 import quant_nf4
from .QFuncs.int8 import quant_int8sym
from .QFuncs.hifxv12 import quant_hifxv12
from .QFuncs.tmx import quant_tmx
...
NPU_KERNELS: Tuple[Tuple[str, NPU_FUNC_BUNDLE_T], ...] = (
                (r'^hifx[0-9]*_v12$', (npu_quant.hifxgv12_quant, npu_quant.hifxgv12_quant_bf16, None),),
...
def get_npu_func(x: Tensor, Q: QType) -> Optional[Callable[[Tensor], Tensor]]:
    def func_wrapper(x: Tensor, func: Callable, cvt_fp32: bool):
        ...
        if Q.q_dim not in (-1, len(x.shape)-1):
...
@torch.no_grad()
def delay_quant_dequant_float(x: Tensor, amax: float, scale_position: float, Q: QType, force_py: bool=False, force_fp32: bool=False, **kwargs) -> Tensor:
    dtype_ori = x.dtype 
    if force_fp32:
    ...
    if 'dhif8' in Q.desc:
        if Q.q_dim not in (-1, len(x.shape)-1):
            x = x.transpose(Q.q_dim, -1).contiguous()
        if x.dtype==torch.bfloat16:
            out = torch.zeros_like(x, dtype=torch.bfloat16, device=x.device)
            npu_quant.delay_hif8_quant_bf16(x.clone(), out, amax, scale_position)
        elif x.dtype==torch.float32:
            out = torch.zeros_like(x, dtype=torch.float32, device=x.device)
            npu_quant.delay_hif8_quant(x.clone(), out, amax, scale_position)
        if Q.q_dim not in (-1, len(x.shape)-1):
            out = out.transpose(Q.q_dim, -1).contiguous()
    else:
