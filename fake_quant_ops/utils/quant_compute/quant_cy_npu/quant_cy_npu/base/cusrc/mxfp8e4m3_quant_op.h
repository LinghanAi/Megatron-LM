...
template <typename T>
class MXFP8E4M3VecKernel{
public: 
    aifunc MXFP8E4M3VecKernel(){}
    aifunc void Init(GM_ADDR xmtx_, GM_ADDR out_, int M_, int N_){
        ...
        // being //
        scaleabsbuf = Tensor<float, PUB>(0, BATCH*2, offset);
        expbuf_rnd  = Tensor<float, PUB>(0, BATCH*2, offset);
        // end //
    ...
    aifunc void Compute(int mn){
        ...
        pipe_barrier(PIPE_V);
        // abs & 0x7F800000 提取 exponent
        vand((__ubuf__ uint16_t*)expbuf.ptr(), (__ubuf__ uint16_t*)expmask.ptr(), (__ubuf__ uint16_t*)absbuf.ptr(), BATCH*2/64, 1, 1, 1, 8, 0, 8);
        pipe_barrier(PIPE_V);

        //===begin==//
        /* 2.1  abs * √2  ----------------------------------------- */
        vmuls(scaleabsbuf.ptr(),     // dst
            absbuf.ptr(),          // src
            1.41421356237f,
            BATCH*2/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);

        /* 2.2  取指数位到 expbuf_rnd  ----------------------------- */
        vand((__ubuf__ uint16_t*)expbuf_rnd.ptr(),
            (__ubuf__ uint16_t*)expmask.ptr(),
            (__ubuf__ uint16_t*)scaleabsbuf.ptr(),
            BATCH*2/64, 1, 1, 1, 8, 0, 8);
        pipe_barrier(PIPE_V);

        vcmax(expmaxbuf.ptr(), expbuf_rnd.ptr(), BATCH*2/64, 1, 1, 8, ONLY_VALUE);
        pipe_barrier(PIPE_V);
        //===end==//

        // get max exp for each block  64 lane 求最大
        ...
        pipe_barrier(PIPE_V);
        // 再 broadcast 回 BATCH*2
        vbrcb((__ubuf__ uint32_t*)grpexpbuf.ptr(), (__ubuf__ uint32_t*)expmaxbrcbbuf.ptr(), 1, 8, BATCH*2/64);
        pipe_barrier(PIPE_V);

        // clip to get private exp 
        vmuls(upperbuf.ptr(), grpexpbuf.ptr(), 1.875f, BATCH*2/64, 1, 1, 8, 8);
        vmuls(lowerbuf.ptr(), grpexpbuf.ptr(), -1.875f, BATCH*2/64, 1, 1, 8, 8);
        vmuls(privexpbuf.ptr(), grpexpbuf.ptr(), 1.0f / 16384.0f, BATCH*2/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vmax(privexpbuf.ptr(), privexpbuf.ptr(), expbuf.ptr(), BATCH*2/64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        vmuls(privexpbuf.ptr(), privexpbuf.ptr(), 0.125f, BATCH*2/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
...
private:
    ...
    Tensor<float, PUB> scaleabsbuf, expbuf_rnd;  //addhx
...
extern "C" __global__ __aicore__ void mxfp8e4m3_kernel(GM_ADDR xmtx, GM_ADDR out, int M, int N){
    if ASCEND_IS_AIV{
#ifdef __DAV_C220_VEC__
        MXFP8E4M3VecKernel<float> vec;
...
extern "C" __global__ __aicore__ void mxfp8e4m3_kernel_bf16(GM_ADDR xmtx, GM_ADDR out, int M, int N){
    if ASCEND_IS_AIV{
#ifdef __DAV_C220_VEC__
        MXFP8E4M3VecKernel<bfloat16_t> vec;
        vec.Init(xmtx, out, M, N);
        vec.Compute();
#endif 
    }
...
void run_mxfp8e4m3_kernel(uint32_t blockDim, void* stream, uint8_t* xmtx, uint8_t* out, int M, int N) {
    mxfp8e4m3_kernel<<<40, nullptr, stream>>>(xmtx, out, M, N);
}

void run_mxfp8e4m3_kernel_bf16(uint32_t blockDim, void* stream, uint8_t* xmtx, uint8_t* out, int M, int N) {
    mxfp8e4m3_kernel_bf16<<<40, nullptr, stream>>>(xmtx, out, M, N);
}