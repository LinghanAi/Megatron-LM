...
public: 
    ...
    aifunc void Init(GM_ADDR xmtx_, GM_ADDR out_, int M_, int N_, int MB_){
        ...
        brcbbuf2 = Tensor<float, PUB>(0, BATCH, offset);
        exp1buf = Tensor<float, PUB>(0, BATCH, offset);
        exp21buf = Tensor<float, PUB>(0, BATCH, offset);
        exp22buf = Tensor<float, PUB>(0, BATCH, offset);
        exp2buf = Tensor<float, PUB>(0, BATCH, offset);
        exp3buf = Tensor<float, PUB>(0, BATCH, offset);
        mant = Tensor<float, PUB>(0, BATCH, offset);
    ...
    aifunc void Compute(int mn){
        ...
        pipe_barrier(PIPE_V);
        vand((__ubuf__ uint16_t*)exp3buf.ptr(), (__ubuf__ uint16_t*)expmask.ptr(), (__ubuf__ uint16_t*)absbuf.ptr(), BATCH/64, 1, 1, 1, 8, 0, 8);
        pipe_barrier(PIPE_V);
        
        // Get exp0
        vcmax(exp0buf.ptr(), exp3buf.ptr(), BATCH/64, 1, 1, 8, ONLY_VALUE);
        pipe_barrier(PIPE_V);
        ...
        pipe_barrier(PIPE_V);
        vmuls(exp0buf.ptr(), exp0buf.ptr(), 0.25f, BATCH/64, 1, 1, 8, 8);   
        pipe_barrier(PIPE_V);
        vadds(exp0buf.ptr(), exp0buf.ptr(), (float)eps, BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);

        // Get exp1
        vcgmax(brcbbuf.ptr(), exp3buf.ptr(), BATCH/64, 1, 1, 8);
        pipe_barrier(PIPE_V);
        vbrcb((__ubuf__ uint32_t*) exp1buf.ptr(), (__ubuf__ uint32_t*) brcbbuf.ptr(), 1, 8, BATCH/64);
        pipe_barrier(PIPE_V);
        vdiv(exp1buf.ptr(), exp1buf.ptr(), exp0buf.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        vmuls(exp1buf.ptr(), exp1buf.ptr(), 0.5f, BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vmaxs(exp1buf.ptr(), exp1buf.ptr(), 1.0f, BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vmul(exp1buf.ptr(), exp0buf.ptr(), exp1buf.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);

        // Get exp2 group1
        copy_ubuf_to_ubuf(exp21buf.vptr(), exp3buf.vptr(), 0, 1, BATCH/8, 0, 0);
        copy_ubuf_to_ubuf(exp22buf.vptr(), exp3buf.vptr(), 0, 1, BATCH/8, 0, 0);
        pipe_barrier(PIPE_V);
        // mask half 
        set_vector_mask(0, 0x0f0f0f0f0f0f0f0f);
        pipe_barrier(PIPE_V);
        vector_dup(exp22buf.ptr(), 0.0f, BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        set_vector_mask(0, 0xf0f0f0f0f0f0f0f0);
        pipe_barrier(PIPE_V);
        vector_dup(exp21buf.ptr(), 0.0f, BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        // compute max
        set_vector_mask((uint64_t)-1, (uint64_t)-1);
        pipe_barrier(PIPE_V);
        vcgmax(brcbbuf.ptr(), exp21buf.ptr(), BATCH/64, 1, 1, 8);
        vcgmax(brcbbuf2.ptr(), exp22buf.ptr(), BATCH/64, 1, 1, 8);
        pipe_barrier(PIPE_V);
        vbrcb((__ubuf__ uint32_t*) exp21buf.ptr(), (__ubuf__ uint32_t*) brcbbuf.ptr(), 1, 8, BATCH/64);
        vbrcb((__ubuf__ uint32_t*) exp22buf.ptr(), (__ubuf__ uint32_t*) brcbbuf2.ptr(), 1, 8, BATCH/64);
        pipe_barrier(PIPE_V);
        // fill 0 and add. Get group-wise max exp 
        set_vector_mask(0, 0x0f0f0f0f0f0f0f0f);
        pipe_barrier(PIPE_V);
        vector_dup(exp22buf.ptr(), 0.0f, BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        ...
        set_vector_mask((uint64_t)-1, (uint64_t)-1);
        pipe_barrier(PIPE_V);
        vadd(exp2buf.ptr(), exp21buf.ptr(), exp22buf.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);

        vdiv(exp2buf.ptr(), exp2buf.ptr(), exp1buf.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        vmaxs(exp2buf.ptr(), exp2buf.ptr(), 1.0f, BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vmul(exp2buf.ptr(), exp1buf.ptr(), exp2buf.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        vmuls(exp2buf.ptr(), exp2buf.ptr(), 1.0f/(float)(1<<(shape.MB-1)), BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);

        // round mantissa 
        vdiv(mant.ptr(), inp_tsr.ptr(), exp2buf.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        vconv_f322f32a(mant.ptr(), mant.ptr(), BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vmins(mant.ptr(), mant.ptr(), (float)((1<<(shape.MB))-1), BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vmaxs(mant.ptr(), mant.ptr(), -(float)((1<<(shape.MB))-1), BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);

        // get final result (mantissa(signed) * exp)
        vmul(out_tsr.ptr(), exp2buf.ptr(), mant.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        if constexpr(std::is_same<T, float>::value){
        }else{
...
private:
    ...
    Tensor<float, PUB> absbuf, exp0buf, brcbbuf, brcbbuf2, exp1buf, exp21buf, exp22buf, exp2buf, exp3buf, mant, fp32inbuf, fp32outbuf; 
...
extern "C" __global__ __aicore__ void hifxg_v12_kernel(GM_ADDR xmtx, GM_ADDR out, int M, int N, int mant_bit){
    if ASCEND_IS_AIV{
...
extern "C" __global__ __aicore__ void hifxg_v12_kernel_bf16(GM_ADDR xmtx, GM_ADDR out, int M, int N, int mant_bit){
    if ASCEND_IS_AIV{
#ifdef __DAV_C220_VEC__
        VecKernel<bfloat16_t> vec;
        vec.Init(xmtx, out, M, N, mant_bit);
        vec.Process();
#endif 
    }
...
void run_hifxg_v12_kernel(uint32_t blockDim, void* stream, uint8_t* xmtx, uint8_t* out, int M, int N, int mant_bit) {
    hifxg_v12_kernel<<<40, nullptr, stream>>>(xmtx, out, M, N, mant_bit);
}

void run_hifxg_v12_kernel_bf16(uint32_t blockDim, void* stream, uint8_t* xmtx, uint8_t* out, int M, int N, int mant_bit) {
    hifxg_v12_kernel_bf16<<<40, nullptr, stream>>>(xmtx, out, M, N, mant_bit);
}
