...
public: 
    ...
    aifunc void Init(GM_ADDR xmtx_, GM_ADDR out_, int M_, int N_){
        ...
        sfbuf = Tensor<float, PUB>(0, CeilDiv(BATCH/16, 64)*64, offset);
        sfexpbuf = Tensor<float, PUB>(0, CeilDiv(BATCH/16, 64)*64, offset);
    ...
    aifunc void Compute(int mn){
        ...
        vmuls(sfbuf.ptr(), sfbuf.ptr(), 1.0f/6.0f, CeilDiv(BATCH/16,64), 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vmins(sfbuf.ptr(), sfbuf.ptr(), 448.0f, CeilDiv(BATCH/16,64), 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        // get exp for scale factor 
        vand((__ubuf__ uint16_t*)sfexpbuf.ptr(), (__ubuf__ uint16_t*)expmask.ptr(), (__ubuf__ uint16_t*)sfbuf.ptr(), CeilDiv(BATCH/16,64), 1, 1, 1, 8, 0, 8);
        pipe_barrier(PIPE_V);
        vmaxs(sfexpbuf.ptr(), sfexpbuf.ptr(), 0.015625f, CeilDiv(BATCH/16,64), 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vmuls(sfexpbuf.ptr(), sfexpbuf.ptr(), 0.125f, CeilDiv(BATCH/16,64), 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        // get mantissa and quantized number 
        vdiv(sfbuf.ptr(), sfbuf.ptr(), sfexpbuf.ptr(), CeilDiv(BATCH/16,64), 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        vconv_f322f32r(sfbuf.ptr(), sfbuf.ptr(), CeilDiv(BATCH/16,64), 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        vmul(sfbuf.ptr(), sfbuf.ptr(), sfexpbuf.ptr(), CeilDiv(BATCH/16,64), 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
