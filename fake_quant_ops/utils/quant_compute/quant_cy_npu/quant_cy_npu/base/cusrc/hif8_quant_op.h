...
//=================================先处理上下溢出的数值========================================
        ...
        vabs(inp_abs_tsr.ptr(),inp_tsr.ptr(),BATCH/64, 1, 1, 8, 8);//取绝对值
        pipe_barrier(PIPE_V);

        // vcmpvs_le((__ubuf__ uint8_t*)leMaxbuf.ptr(), inp_abs_tsr.ptr(), *f_max, BATCH/64, 1, 1, 8, 8);//把上溢的值变为最大值
        vcmpvs_lt((__ubuf__ uint8_t*)posinfbuf.ptr(), inp_tsr.ptr(), 40960.0f, BATCH/64, 1, 1, 8, 8);//超过+/-2^15 * 1.25的值就超出Hif8能表示的范围，变为正负无穷的值
        vcmpvs_gt((__ubuf__ uint8_t*)neginfbuf.ptr(), inp_tsr.ptr(), -40960.0f, BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
        //vcmpvs_ge((__ubuf__ uint8_t*)geMaxbuf.ptr(), inp_tsr.ptr(), -*f_max, BATCH/64, 1, 1, 8, 8);
        vcmpvs_lt((__ubuf__ uint8_t*)ltzerobuf.ptr(), inp_tsr.ptr(), 0.0f, BATCH/64, 1, 1, 8, 8);//把负数过滤出来
        vcmpvs_ge((__ubuf__ uint8_t*)eq0buf.ptr(), inp_abs_tsr.ptr(), *f_min, BATCH/64, 1, 1, 8, 8); //对小于2^-23的数归0,会在最后做
        vcmpvs_ge((__ubuf__ uint8_t*)geminbuf.ptr(), inp_abs_tsr.ptr(), *f_exp_min, BATCH/64, 1, 1, 8, 8);//对于小于2^-22的数全归为2^-22
       // vcmpvs_ne((__ubuf__ uint8_t*)eq0buf.ptr(), inp_abs_tsr.ptr(), 0.0f, BATCH/64, 1, 1, 8, 8);//0值处理
        pipe_barrier(PIPE_V);
    ...
    //     //做一个全是-1和1的张量sign，原来是负数的位置为-1
        ...
        pipe_barrier(PIPE_V);
    //     //处理上溢出的值
        // set_cmpmask(maxbuf.vptr());
        // pipe_barrier(PIPE_V);
        // vsel(inp_abs_tsr.ptr(), inp_abs_tsr.ptr(), leMaxbuf.vptr(), BATCH/64, 1, 1, 1, 8, 8, 1, 1);
        // pipe_barrier(PIPE_V);

    //     //处理小于2^-22的值
        set_cmpmask(minbuf.vptr());
        pipe_barrier(PIPE_V);
        vsel(inp_abs_tsr.ptr(), inp_abs_tsr.ptr(), geminbuf.vptr(), BATCH/64, 1, 1, 1, 8, 8, 1, 1);
        pipe_barrier(PIPE_V);

        vmul(inp_tsr.ptr(), inp_abs_tsr.ptr(), sign.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);//将符号乘回去
        pipe_barrier(PIPE_V);
...
//===================指数处理=====================
        ...
        // vmaxs(expbuf.ptr(), expbuf.ptr(), *f_exp_min, BATCH/64, 1, 1, 8, 8);//指数的下限夹取，感觉不太对，尾数也会有影响的。
        // pipe_barrier(PIPE_V);
        // ---- get abs exp ----
...
private:
    ...
    Tensor<float, PUB> inp_abs_tsr, expbuf, expabsbuf, le15buf, le7buf, le3buf, eq0buf, twosbuf, onesbuf, zerosbuf, expbias1, expbias2, expbias3, maxbuf,leMaxbuf;
    Tensor<float, PUB> minbuf, geminbuf, sign, n_one, ltzerobuf, posinfbuf, neginfbuf;
