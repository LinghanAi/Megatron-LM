...
public:
    aifunc Hif8DelayVec(){}
    aifunc void Init(GM_ADDR src, GM_ADDR dst, float amax, float scale_position, int len){
        ...
        // amaxbuf = Tensor<float, PUB>(0, BATCH, offset);
        ...
        scaler = scale_position/amax;
        vector_dup(scalebuf.ptr(), scaler, BATCH/64, 1, 1, 8, 8);
    ...
    aifunc void Compute(int &n, int &cnt){
        ...
        // uint32_t scaler = 0x3E8A3D71; //把最大值缩放到15，这是1/15的值
        // float *f_scaler =(float *)&scaler;

//=================================quant process========================================
        // ---- get exp ---- 
        // vand((__ubuf__ uint16_t*)grpexpbuf.ptr(), (__ubuf__ uint16_t*)amaxbuf.ptr(), (__ubuf__ uint16_t*)expmask.ptr(), BATCH/64, 1, 1, 1, 8, 8, 0);
        // pipe_barrier(PIPE_V);
        //除0保护
        // vcmpvs_ne((__ubuf__ uint8_t*)eq0buf.ptr(), amaxbuf.ptr(), 0.0f, BATCH/64, 1, 1, 8, 8);
        // pipe_barrier(PIPE_V);
        ...
        // pipe_barrier(PIPE_V);
        //量化后的最大值取8, 1/8 = 0.125
        // vmuls(scalebuf.ptr(), grpexpbuf.ptr(), *f_scaler, BATCH/64, 1, 1, 8, 8);
        // pipe_barrier(PIPE_V);
        // vdiv(scaled_inp_tsr.ptr(), inp_tsr.ptr(), scalebuf.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);
        vmuls(scaled_inp_tsr.ptr(), inp_tsr.ptr(), scaler, BATCH/64, 1, 1, 8, 8);
        pipe_barrier(PIPE_V);
...
//===================对结果上下溢出处理================================
        ...
        vcmpvs_le((__ubuf__ uint8_t*)posinfbuf.ptr(), out_abs_tsr.ptr(), 40960.0f, BATCH/64, 1, 1, 8, 8);//超过+/-2^15 * 1.25的值变为2^15
        ...
        // fill posinf 处理正无穷的值
        set_cmpmask(maxbuf.vptr());//正无穷改为最大值
        pipe_barrier(PIPE_V);
...
//==========================dequant process===============================
        ...
        vdiv(out_tsr.ptr(), out_tsr.ptr(), scalebuf.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
...
private:
    ...
    Tensor<float, PUB> grpexpbuf, epsbuf, scaled_inp_tsr, scalebuf, maxbuf;
...
extern "C" __global__ __aicore__ void delay_hif8_kernel(GM_ADDR xmtx, GM_ADDR out, float amax, float scale_position, int len){
    if ASCEND_IS_AIV{
#ifdef __DAV_C220_VEC__
        Hif8DelayVec<float> vec;
        vec.Init(xmtx, out, amax, scale_position, len);
        vec.Compute();
#endif 
    }
}

extern "C" __global__ __aicore__ void delay_hif8_kernel_bf16(GM_ADDR xmtx, GM_ADDR out, float amax, float scale_position, int len){
    if ASCEND_IS_AIV{
#ifdef __DAV_C220_VEC__
        Hif8DelayVec<bfloat16_t> vec;
        vec.Init(xmtx, out, amax, scale_position, len);
        vec.Compute();
#endif 
    }
...
void run_delay_hif8_kernel(uint32_t blockDim, void* stream, uint8_t* xmtx, uint8_t* out, float amax, float scale_position, int len) {
    delay_hif8_kernel<<<40, nullptr, stream>>>(xmtx, out, amax, scale_position, len);
}
...
void run_delay_hif8_kernel_bf16(uint32_t blockDim, void* stream, uint8_t* xmtx, uint8_t* out, float amax, float scale_position, int len) {
    delay_hif8_kernel_bf16<<<40, nullptr, stream>>>(xmtx, out, amax, scale_position, len);
}