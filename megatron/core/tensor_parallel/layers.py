...
class CustomLinearWithGradAccumulationAndAsyncCommunication(torch.autograd.Function):
    """See custom_linear_with_grad_accumulation_and_async_allreduce"""
    ...
    def forward(
        ...
        if custom_quant_type == 'mxfp4':
            # 打印确认 minus_exp 参数生效
            if not hasattr(ctx, '_minus_exp_mxfp4_printed'):
                print(f"[MXFP4 Quantization] Using minus_exp=0 (layer_idx={layer_idx}, rank={rank})")
                ctx._minus_exp_mxfp4_printed = True
            output = mxfp_matmul(
